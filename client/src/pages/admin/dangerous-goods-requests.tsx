...
export default function AdminDangerousGoodsRequests() {
  ...
  const canApproveReq = adminAccess.hasPermission("dangerous-goods-requests", "approve");
  const canRejectReq = adminAccess.hasPermission("dangerous-goods-requests", "reject");
  const canRevokeReq = adminAccess.hasPermission("dangerous-goods-requests", "revoke");

  const { data, isLoading } = useQuery<any>({
    queryKey: ["/api/admin/dangerous-goods-requests", statusFilter],
    queryFn: async () => {
      const params = new URLSearchParams();
      if (statusFilter !== "all") params.set("status", statusFilter);
      const res = await fetch(`/api/admin/dangerous-goods-requests?${params.toString()}`, { credentials: "include" });
  ...
  const actionMutation = useMutation({
    mutationFn: async ({ id, action, notes }: { id: string; action: string; notes: string }) => {
      const res = await apiRequest("POST", `/api/admin/dangerous-goods-requests/${id}/${action}`, { adminNotes: notes });
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/dangerous-goods-requests"] });
      setSelectedRequest(null);
      setActionType(null);
      setAdminNotes("");
      toast({ title: "Success", description: "Dangerous goods request updated." });
    },
    onError: (error) => {
  ...
  if (isLoading) return <AdminLayout><LoadingScreen message="Loading feature requests..." /></AdminLayout>;

  return (
    <AdminLayout>
      <div className="p-6 space-y-6">
        <div>
          <h1 className="text-2xl font-bold" data-testid="text-page-title">Dangerous Goods Requests</h1>
          <p className="text-muted-foreground">Approve clients to ship regulated goods on our carrier accounts</p>
        </div>
          ...
          <CardContent>
            {!data?.requests?.length ? (
              <div className="text-center py-12 text-muted-foreground">
                ...
                <p>No dangerous goods requests found</p>
              </div>
            ) : (
              <Table>
                <TableHeader>
                        ...
                        <div className="flex gap-2">
                          {request.status === "pending" && (
                            <>
                              ...
                              {!canApproveReq && !canRejectReq && (
                                <span className="text-sm text-muted-foreground">Read only</span>
                              )}
                            </>
                          )}
                          {request.status === "approved" && canRevokeReq && (
                            <Button
                              size="sm"
                              variant="outline"
                          ...
                          {(request.status === "rejected" || request.status === "revoked") && canApproveReq && (
                            <Button
                              size="sm"
                              variant="default"
                              onClick={() => openAction(request, "approve")}
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2">
                {actionType === "approve" && <><ShieldCheck className="h-5 w-5 text-green-500" />Approve Dangerous Goods Access</>}
                {actionType === "reject" && <><AlertTriangle className="h-5 w-5 text-red-500" />Reject Dangerous Goods Request</>}
                {actionType === "revoke" && <><ShieldX className="h-5 w-5 text-amber-500" />Revoke Dangerous Goods Access</>}
              </DialogTitle>
              <DialogDescription>
                {actionType === "approve" && `This will let ${selectedRequest?.clientName} declare and ship regulated goods on our carrier accounts. Approve only against a current dangerous goods training certificate and the relevant safety data sheets.`}
                {actionType === "reject" && `This will deny the dangerous goods request from ${selectedRequest?.clientName}.`}
                {actionType === "revoke" && `This will stop ${selectedRequest?.clientName} from declaring dangerous goods. Shipments already in review are unaffected.`}
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-3 py-2">
              <div className="rounded-lg bg-muted p-3 space-y-1">
                ...
                {/* The evidence is the decision. Approving without it means taking on
                    shipper-of-record liability for a shipper we have not verified. */}
                <div className="text-sm">
                  <span className="font-medium">Evidence:</span>{" "}
                  {selectedRequest?.documentPaths?.length
                    ? `${selectedRequest.documentPaths.length} document(s) attached`
                    : "none attached"}
                </div>
              </div>
              <div>
