...
export default function AdminSalesFeatureRequests() {
  ...
  const canApproveReq = adminAccess.hasPermission("sales-feature-requests", "approve");
  const canRejectReq = adminAccess.hasPermission("sales-feature-requests", "reject");
  const canRevokeReq = adminAccess.hasPermission("sales-feature-requests", "revoke");

  const { data, isLoading } = useQuery<any>({
    queryKey: ["/api/admin/sales-feature-requests", statusFilter],
    queryFn: async () => {
      const params = new URLSearchParams();
      if (statusFilter !== "all") params.set("status", statusFilter);
      const res = await fetch(`/api/admin/sales-feature-requests?${params.toString()}`, { credentials: "include" });
  ...
  const actionMutation = useMutation({
    mutationFn: async ({ id, action, notes }: { id: string; action: string; notes: string }) => {
      const res = await apiRequest("POST", `/api/admin/sales-feature-requests/${id}/${action}`, { adminNotes: notes });
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/sales-feature-requests"] });
      setSelectedRequest(null);
      setActionType(null);
      setAdminNotes("");
      toast({ title: "Success", description: "Feature request updated." });
    },
    onError: (error) => {
  ...
  return (
    <AdminLayout>
      <div className="p-6 space-y-6">
        <div>
          <h1 className="text-2xl font-bold" data-testid="text-page-title">Sales Channels Feature Requests</h1>
          <p className="text-muted-foreground">Manage client requests to unlock Orders, Sales Channels & Assignment Rules</p>
        </div>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2">
                {actionType === "approve" && <><ShieldCheck className="h-5 w-5 text-green-500" />Approve Feature Access</>}
                {actionType === "reject" && <><AlertTriangle className="h-5 w-5 text-red-500" />Reject Feature Request</>}
                {actionType === "revoke" && <><ShieldX className="h-5 w-5 text-amber-500" />Revoke Feature Access</>}
              </DialogTitle>
              <DialogDescription>
                {actionType === "approve" && `This will enable Sales Channels for ${selectedRequest?.clientName}.`}
                {actionType === "reject" && `This will deny the feature request from ${selectedRequest?.clientName}.`}
                {actionType === "revoke" && `This will disable Sales Channels for ${selectedRequest?.clientName}. They will lose access to Orders, Sales Channels and Assignment Rules.`}
              </DialogDescription>
            </DialogHeader>
