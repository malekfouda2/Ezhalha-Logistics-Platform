...
export function useShipperStep() {
  const router = useRouter();
  const { t } = useTranslation();
  const [selectedAddressId, setSelectedAddressId] = useState<string | null>(null);
  const hasPrefilled = useRef(false);

  const shipper = useDangerousGoodsStore((s) => s.shipper);
  const setShipper = useDangerousGoodsStore((s) => s.setShipper);
  ...
  const handleContinue = form.handleSubmit(
    (values) => {
      ...
      setShipper(address);
      router.push("/createShipment/dangerousGoods/step-3");
    },
    (errors) => {
  ...
  useEffect(() => {
    if (hasPrefilled.current || isLoadingAddresses || shipper.name) return;
    const defaultAddress = addressBookEntries.find(
      (e) => e.source === "default_shipping" && e.useForShipper,
    );
